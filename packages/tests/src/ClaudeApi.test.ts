...
describe("Claude proxy compatibility API", () => {
  ...
  test("retries a Kimi turn that stays active without completing", async () => {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (_url: string, init?: RequestInit) => {
        ...
        upstreamBodies.push(JSON.parse(String(init.body)) as Record<string, unknown>);
        return upstreamBodies.length === 1
          ? activeReasoningSseResponse()
          : sseResponse([
              { choices: [{ delta: { content: "Recovered after active turn timeout." } }] },
              { choices: [{ finish_reason: "stop", delta: {} }] },
            ]);
      }),
    );
    ...
    expect(upstreamBodies).toHaveLength(2);
    expect(response.status).toBe(200);
    expect(response.body).toContain("Recovered after active turn timeout.");
    expect(response.body).toContain("message_stop");
    expect(response.body).not.toContain("event: error");
  }, 2_500);
...
function activeReasoningSseResponse(): Response {
  const encoder = new TextEncoder();
