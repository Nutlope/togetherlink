...
async function main() {
  ...
  // One-time migration: install the launchd agent on macOS for existing
  // installed bundles. Run only for real commands that may need the daemon
  // (harnesses, chatgpt, daemon subcommands), so read-only commands such as
  // --version and whoami stay side-effect free.
