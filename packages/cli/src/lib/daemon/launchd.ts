...
async function runningFromBundle(): Promise<boolean> {
  ...
  if (argv1) {
    try {
      ...
      if (homePaths.has(resolved)) return true;
    } catch {
      // fall through
    }
  }

  // Fallback: if the runtime executable itself lives inside the bundle home,
  // we are almost certainly the installed binary (e.g. invoked via a symlink).
  try {
    const exec = await realpath(process.argv[0] ?? process.execPath);
    if (exec.startsWith(home + path.sep)) return true;
  } catch {
    // ignore
  }
