...
export function buildPrimeProviderConfig(
  ...
  return {
    ...
    api: "openai-completions",
    models: modelSelections.map(({ definition }) => {
...
export function buildPrimeProviderExtensionSource(
  baseUrl = TOGETHER_BASE_URL,
  modelSelections: readonly CodexModelSelection[] = CODEX_SUPPORTED_MODELS,
): string {
  const config = JSON.stringify(buildPrimeProviderConfig(baseUrl, modelSelections), null, 2);
...
export function resolvePrimeProviderExtensionPath(
  ...
  const endpointHash = createHash("sha256")
    .update(buildPrimeProviderExtensionSource(baseUrl, modelSelections))
    .digest("hex")
    .slice(0, 12);
...
export async function writePrimeProviderExtension(
  ...
  await writeFile(temporaryPath, buildPrimeProviderExtensionSource(baseUrl, modelSelections), {
    mode: 0o600,
  });
...
export function buildPrimeLaunchSpec({
  ...
  const supportedModels = models.map(({ id }) => `${PRIME_PROVIDER_ID}/${id}`).join(",");
  return {
    binary: "prime-agent",
    args: [
      "--extension",
