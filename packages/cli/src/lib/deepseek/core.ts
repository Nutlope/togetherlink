...
export function buildDeepseekPatchSource(
  ...
  return `# Generated by TogetherLink. Contains model metadata only; no credentials.\n${stringify(
    buildDeepseekPatch(selectedModel, baseUrl, nativeDeepseekApiKey, modelSelections),
  )}`;
}
  ...
  const contentHash = createHash("sha256")
    .update(
      buildDeepseekPatchSource(
        selectedModel,
        baseUrl,
        env[NATIVE_DEEPSEEK_API_KEY_ENV],
        modelSelections,
      ),
    )
    .digest("hex")
...
export async function writeDeepseekPatch(
  ...
  await writeFile(
    temporaryPath,
    buildDeepseekPatchSource(selectedModel, baseUrl, nativeDeepseekApiKey, modelSelections),
    { mode: 0o600 },
  );
