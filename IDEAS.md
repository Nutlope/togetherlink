...
- **Live connection reuse diagnostic**: rejected as a keep-alive tuning trigger for now. The opt-in real-network command `TOGETHERLINK_LIVE_CONNECTION_BENCH=1 pnpm -F @togetherlink/tests exec vitest run --config vitest.bench.config.ts benchmarks/live-proxy-performance.bench.ts --reporter verbose` hits Together `/models` without generation credits. Five measured calls after one warmup showed default Node fetch at p50 494.743 ms / mean 466.580 ms and forced `Connection: close` at p50 386.552 ms / mean 467.620 ms. The means were effectively identical and p50 favored the forced-close row in this noisy sample, so this does not justify another keep-alive implementation after the dispatcher incompatibility.
...
| Keep-alive / connection reuse tuning for Together fetches                | Reduce per-request handshake latency                                    | Low code risk, **validation risk** (mocked benches hide it) | Live `/models` connection diagnostic + local validation      | Rejected; no clear live win               |
