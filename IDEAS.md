...
- **Live connection reuse diagnostics**: rejected as a keep-alive tuning trigger for now, but the real generation test is now the primary evidence. The cheap `/models` command `TOGETHERLINK_LIVE_CONNECTION_BENCH=1 pnpm -F @togetherlink/tests exec vitest run --config vitest.bench.config.ts benchmarks/live-proxy-performance.bench.ts --reporter verbose` showed default Node fetch at p50 494.743 ms / mean 466.580 ms and forced `Connection: close` at p50 386.552 ms / mean 467.620 ms, so it was too noisy to prove anything by itself. The stronger paired generation benchmark `TOGETHERLINK_LIVE_GENERATION_CONNECTION_BENCH=1 pnpm -F @togetherlink/tests exec vitest run --config vitest.bench.config.ts benchmarks/live-proxy-performance.bench.ts --reporter verbose` alternates default vs forced-close ordering and runs both sequential and 3-way concurrent real model calls. In that run, sequential proxy generation favored default fetch (default p50 578.817 ms / mean 589.131 ms; forced close p50 620.641 ms / mean 1292.971 ms). Direct concurrent generation also favored default (default p50 3460.426 ms / mean 3841.265 ms; forced close p50 5025.190 ms / mean 4941.005 ms). Proxy concurrent was mixed in the broad paired run (forced close p50 1608.022 ms / mean 2028.536 ms vs default p50 2084.363 ms / mean 2982.111 ms), so I reran a focused 8-iteration proxy-concurrent confirmation: default batch p50 2272.737 ms / mean 2901.357 ms, forced-close batch p50 1819.487 ms / mean 2894.291 ms, close-minus-default mean -7.066 ms. That is effectively neutral at batch level, and per-call upstream spans favored default on mean and p95 (default upstream p50 1141.038 ms / mean 1736.772 ms / p95 4287.093 ms; forced close p50 1194.344 ms / mean 2128.344 ms / p95 8292.919 ms). Conclusion: default fetch pooling is not proven worse under concurrent real generation, forced close is not a safe optimization, and the previous custom dispatcher remains reverted.
...
- **Live Together proxy timing gate**: confirmed with real API calls using `TOGETHERLINK_LIVE_PROXY_BENCH=1 pnpm -F @togetherlink/tests exec vitest run --config vitest.bench.config.ts benchmarks/live-proxy-performance.bench.ts --reporter verbose`. Five measured calls after one warmup showed direct buffered p50 800.886 ms / mean 755.625 ms and Claude proxy buffered p50 650.027 ms / mean 753.947 ms. Direct stream TTFT p50 was 542.648 ms / mean 639.276 ms; Claude proxy stream TTFT p50 was 616.495 ms / mean 1521.638 ms, with one live upstream outlier. Proxy perf spans across warmup + measured proxy calls showed `body_read_parse` p50 0.111 ms, `translate_request` p50 0.023 ms, and `upstream_fetch` p50 613.197 ms. This real-call timing confirms local translation is not the first-order bottleneck right now.
- **Common proxy fast paths**: first pass retained, further generic fast-path work rejected for now. The isolated change skipped vision resolution when there are no image blocks, reused an empty Codex tool translation, and delayed debug payload construction until debug logging is active. The per-commit benchmark series was mixed: captured Codex was neutral/slightly noisy (p50 0.233 -> 0.242 ms, p95 0.324 -> 0.353 ms), captured Claude improved (p50 1.129 -> 0.972 ms, p95 1.577 -> 1.286 ms), and large synthetic rows were noisy. The latest full mocked run stayed tiny on captured payloads (Codex p50 0.238 ms / p95 0.330 ms; Claude p50 0.931 ms / p95 1.374 ms), while the live benchmark showed `translate_request` p50 0.023 ms. Keep the simple guards because they are low-risk, but do not add more generic fast paths until perf telemetry shows translation is a material share of real request time.
- **Per-commit benchmark series** (`635e552` baseline → `8206a9e` current, `pnpm bench:proxy`, mocked upstream):
  ...
  - Common fast paths: retained as low-risk guards, but further generic fast-path work is rejected by current mocked + live timing.
...
| Fast-path common case (no images, no native tools, no special reasoning) | Reduce object churn and work on the 80-90% path                         | Low (if guarded)                                            | Mocked captured payloads + live proxy timing                 | Retain first pass; stop generic work      |
| Keep-alive / connection reuse tuning for Together fetches                | Reduce per-request handshake latency                                    | Low code risk, **validation risk** (mocked benches hide it) | Live generation connection diagnostics + local validation    | Rejected; no stable live win              |
...
| Worker threads for translation / Rust thin layer / HTTP/2 to Together    | Big wins on CPU-bound or connection paths                               | High (complexity, maintenance)                              | Mocked + live proxy timing                                   | Rejected by live timing gate              |
...
- **Keep-alive tuning**: Rejected for now. The custom dispatcher failed local validation, and paired live generation diagnostics did not show a stable forced-close or custom-keepalive win over default fetch pooling.
