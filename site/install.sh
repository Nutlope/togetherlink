...
# PATH. Also installs `tclaude`, `topencode`, `tcodex`, `tgrok`, `tomp`, and `tpi` convenience wrappers.
#
...
# Short aliases: tclaude / topencode / tcodex / tgrok / tomp / tpi
...
cat > "$BIN_DIR/tomp" <<EOF
#!/usr/bin/env sh
exec bun "$BIN_DIR/togetherlink.js" omp "\$@"
EOF
chmod +x "$BIN_DIR/tomp"
...
ok "Wrappers installed: togetherlink, tclaude, topencode, tcodex, tgrok, tomp, tpi → $BIN_DIR"
...
# should get the real CLIs unless they explicitly run tclaude/tcodex/topencode/tgrok/tomp/tpi.
remove_legacy_shadow_wrapper() {
  ...
  if [ -L "$path" ]; then
    ...
    case "$target" in
      "$BIN_DIR/tclaude"|"$BIN_DIR/tcodex"|"$BIN_DIR/topencode"|"$BIN_DIR/tgrok"|"$BIN_DIR/tomp"|"$BIN_DIR/tpi"|"$BIN_DIR/togetherlink"|"$BIN_DIR/togetherlink.js")
        rm -f "$path"
...
if LINK_DIR="$(find_writable_path_dir)"; then
  ...
  install_link tomp "$BIN_DIR/tomp"
