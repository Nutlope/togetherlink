...
# PATH. Also installs `tclaude`, `topencode`, `tcodex`, `tdeepseek`, `tdroid`,
...
# Short aliases: tclaude / topencode / tcodex / tdeepseek / tdroid / tgrok / thermes / tpi / tprime
...
cat > "$BIN_DIR/tdroid" <<EOF
#!/usr/bin/env sh
exec bun "$BIN_DIR/togetherlink.js" droid "\$@"
EOF
chmod +x "$BIN_DIR/tdroid"
...
ok "Wrappers installed: togetherlink, tclaude, topencode, tcodex, tdeepseek, tdroid, tgrok, thermes, tpi, tprime → $BIN_DIR"
...
remove_legacy_shadow_wrapper() {
  ...
  if [ -L "$path" ]; then
    ...
    case "$target" in
      "$BIN_DIR/tclaude"|"$BIN_DIR/tcodex"|"$BIN_DIR/tdeepseek"|"$BIN_DIR/tdroid"|"$BIN_DIR/topencode"|"$BIN_DIR/tgrok"|"$BIN_DIR/thermes"|"$BIN_DIR/tpi"|"$BIN_DIR/tprime"|"$BIN_DIR/togetherlink"|"$BIN_DIR/togetherlink.js")
        rm -f "$path"
...
if LINK_DIR="$(find_writable_path_dir)"; then
  ...
  install_link tdroid "$BIN_DIR/tdroid"
